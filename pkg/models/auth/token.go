...
import (
	...
	"devops.kubesphere.io/plugin/pkg/apiserver/authentication/oauth"
	authoptions "devops.kubesphere.io/plugin/pkg/apiserver/authentication/options"
	"devops.kubesphere.io/plugin/pkg/apiserver/authentication/token"
...
func (t tokenOperator) Verify(tokenStr string) (user.Info, error) {
	authenticated, _, err := t.issuer.Verify(tokenStr)
	if err != nil {
		return nil, err
	}
	// the plugin only takes request from ks-apiserver, so it's not necessary to check it
